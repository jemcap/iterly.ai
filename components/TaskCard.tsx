    ...
    <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
      ...
      {task.aiAnalysis && (
        <div className="flex flex-wrap gap-1 mb-3">
          {/* Category */}
          <span className={`px-2 py-1 rounded-full text-xs font-medium ${getCategoryColor(task.aiAnalysis.category)}`}>
            {task.aiAnalysis.category}
          </span>
          
          {/* Action Type */}
          <span className="px-2 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
            {getActionTypeIcon(task.aiAnalysis.actionType)} {task.aiAnalysis.actionType}
          </span>
          
          {/* Estimated Effort */}
          <span className={`px-2 py-1 rounded-full text-xs font-medium ${getEffortColor(task.aiAnalysis.estimatedEffort)}`}>
